...
func init() {
	android.RegisterModuleType("citric_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = citricExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = citricExpandVariables(ctx, dep_file)
	...
	exCmd := citricExpandVariables(ctx, String(g.properties.Cmd))
